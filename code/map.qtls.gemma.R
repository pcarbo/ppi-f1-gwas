...
for (which.analysis in names(model.info)) {
  ...
  # We only want the wild-types.
  choose.samples <- "wt"
  
  # Create folders for GEMMA and specify some model information
  system(paste0("mkdir ~/gemma_output/",which.analysis))
  gemmadir <- paste0("~/gemma_output/",which.analysis,"/",choose.samples)
  system(paste0("mkdir ",gemmadir))
  
  # Name of final result .RData file.
  resultsfile <-
    paste0("~/wt_combined_gemma/",which.analysis,".wtcombined.RData")
