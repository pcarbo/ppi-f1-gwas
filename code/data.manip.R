...
# ----------------------------------------------------------------------
# Transforms the given phenotype with the given transformation
# function Returns an updated prepared.pheno with the phenotype's old
# column replaced by a transformed version
transform.pheno <- function (prepared.pheno, phenotype, transformation,
                             verbose = TRUE) {
  ...
  if (is.null(transformation)) {
    if (verbose)
      cp0("Did not transform ", phenotype," because there is no",
  ...
  if (verbose)
    cp0("Transformed ", phenotype, ".\n")
  prepared.pheno[[phenotype]] <- transformation(prepared.pheno[[phenotype]])
  
  # Return the modified phenotype data table.
  return(prepared.pheno)
}

# ----------------------------------------------------------------------
# Removes outliers in a given phenotype based on the given outlier
# function Note that the function assumes transformed data is given.
# This function removes outliers based on the outlier function and
# based on outliers given by ID Returns an updated transformed
# phenotype with a NA assigned to each outlying observation.
remove.outliers <- function (transformed.pheno, phenotype, covariates,
  ...
  if (is.null(outlier.function)) {
    if (verbose) cp0("Did not apply an outlier removal function on ",
                     phenotype," (none given).\n")
  } else {

    # There is an outlier function. Create a function that tells if x
    # is an outlier (takes into account NAs).
    ...
    # Check the residuals for outliers and report the number that will
    # be removed.
    if (verbose) {
      n <- sum(is.outlier(r))
      if (n == 0)
  ...
  if (length(outliers) > 0) {
    ...
    # No ID specified outliers to remove.
    if (verbose)
      cp0("Did not remove any ID specified outliers (none given).\n")
